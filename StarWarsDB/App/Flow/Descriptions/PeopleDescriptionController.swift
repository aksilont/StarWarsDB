//
//  PeopleDescriptionController.swift
//  StarWarsDB
//
...
class PeopleDescriptionController: UITableViewController {
    
    private var elements = [DescriptionElement]()
    
    func setObjectId(_ id: Int16) {
        DataRepository.shared.fetchPeople(with: id) { [weak self] people in
            self?.elements = DescriptionBuilder().build(from: people)
